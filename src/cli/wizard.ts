...
import type { DarkModeMode, FeatureFlags, MdgardenConfig } from '../types.js';
...
export async function promptThemeSelection(
  ...
  const darkMode = await p.select({
    message: 'Dark mode',
    options: [
      { value: 'auto', label: 'Auto', hint: 'follow the OS preference (default)' },
...
async function writeConfig(cwd: string, config: MdgardenConfig): Promise<string> {
...
export function defaultConfig(cwd: string, defaults: WizardDefaults = {}): MdgardenConfig {
  return composeConfig({
    ...
    darkMode: defaults.darkMode ?? 'auto',
